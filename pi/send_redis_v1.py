...
Q = False
LOCK = False
LFRAME = cv2.imread('/home/pi/tflite1')
...
#luong lay du lieu camera

def getImgThread():
    global Q, LOCK, LFRAME
    
    # Luu du lieu de truyen sang M1 - frame da detect
    makeFolder('buffer')
    # Luu tru du lieu hinh anh 
    makeFolder('data_img')
    tpf = 1/FPS
    cnt = 0
    pre_t = getTime()
    while not Q:
        ...
        if curr_t - pre_t >tpf:
            ...
            if k == ord("q"):
                Q = True
            LOCK = True
            cv2.imwrite(os.path.join('data_img', str(curr_t)+'.jpg'), img)
            #cv2.imwrite(os.path.join('buffer', str(curr_t)+'.jpg'), img)
            LFRAME = img
            cv2.imshow("a", LFRAME)
            #k = cv2.waitKey(1)
            LOCK = False
    ...
    for image_path in images:
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and (int(classes[i]) == 0)):
                ...
                if(int(classes[i]) == 0 and scores[i]*100 >= 20):
                    isHuman = True
                # Draw label
